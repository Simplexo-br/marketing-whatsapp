...
with registry.cursor() as cr:
    ...
    print("==> Atualizando lista de modulos no banco simplexo...")
    env['ir.module.module'].update_list()
    cr.commit()

    mod = env['ir.module.module'].search([('name', '=', 'marketing_whatsapp')])
    if mod:
        print(f"==> Modulo encontrado! Nome: {mod.name}, Estado: {mod.state}, Titulo: {mod.shortdesc}")
    else:
        print("==> Modulo marketing_whatsapp ainda nao encontrado.")
"""
...
stdout, stderr = proc.communicate(input=update_script, timeout=60)
