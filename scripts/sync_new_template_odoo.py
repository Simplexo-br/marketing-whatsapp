...
with registry.cursor() as cr:
    ...
    if tmpl:
        print(f"Template sincronizado no Odoo! ID={tmpl.id}, Status={tmpl.status}, Header={tmpl.header_type}, Botoes={len(tmpl.button_ids)}")
        for b in tmpl.button_ids:
            print(f"  Botao: {b.name} ({b.button_type}) -> {b.url or b.payload}")
    else:
        print("Template simplexo_gestao_completa_v1 ainda nao encontrado no Odoo.")
"""
