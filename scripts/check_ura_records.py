...
cr.execute(\"\"\"
    SELECT id, name, active, enable_greeting, enable_ura, routing_mode, greeting_message
    FROM simplexo_aios_whatsapp_ura_config;
\"\"\")
configs = cr.fetchall()
print(f"Total URA Configs: {len(configs)}")
for c in configs:
    print(c)
