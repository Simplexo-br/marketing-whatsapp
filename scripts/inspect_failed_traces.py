...
with registry.cursor() as cr:
    ...
    failed = Trace.search([('mass_mailing_id', '=', 134), ('whatsapp_status', '=', 'failed')], limit=5)
    for f in failed:
        print(f"Failed Trace ID: {f.id} | Phone: {f.whatsapp_recipient_number} | Code: {f.whatsapp_error_code} | Msg: {f.whatsapp_error_message}")
        
    sent = Trace.search([('mass_mailing_id', '=', 134), ('whatsapp_status', '=', 'sent')], limit=5)
    for s in sent:
        print(f"Sent Trace ID: {s.id} | Phone: {s.whatsapp_recipient_number} | WAMID: {s.whatsapp_message_id}")
"""
