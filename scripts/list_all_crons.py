...
with registry.cursor() as cr:
    ...
    crons = Cron.search(['|', '|', '|', '|',
        ...
        ('model_name', 'ilike', 'whatsapp')
    ])
    
    print("=== TODOS OS CRONS RELEVANTES ===")
    for c in crons:
        print(f"ID: {c.id} | Ativo: {c.active}")
        print(f"   Nome: {c.name}")
        print(f"   Modelo: {c.model_name}")
        print(f"   Codigo/Metodo: {c.code}")
        print(f"   Intervalo: a cada {c.interval_number} {c.interval_type}")
        print(f"   Proxima Execucao (UTC): {c.nextcall}")
        print("-" * 50)
"""
