...
print("=== BUSCA PROCESSAMENTO DO EVENTO DE WEBHOOK ===")
res = subprocess.run("grep -rn 'class .*WebhookEvent' /opt/odoo/addons/simplexo_aios_whatsapp/", shell=True, capture_output=True, text=True)
print(res.stdout)
res2 = subprocess.run("grep -rn '_name = .simplexo.aios.whatsapp.webhook.event.' /opt/odoo/addons/simplexo_aios_whatsapp/", shell=True, capture_output=True, text=True)
print(res2.stdout)
"""
