...
print("=== BUSCANDO CONTROLLER DO WEBHOOK ===")
res = subprocess.run("grep -rn 'simplexo-aios/whatsapp/webhook' /opt/odoo/", shell=True, capture_output=True, text=True)
print(res.stdout)
if not res.stdout:
    res2 = subprocess.run("grep -rn 'YTfh0e_DdpJ5' /opt/odoo/", shell=True, capture_output=True, text=True)
    print(res2.stdout)
"""
