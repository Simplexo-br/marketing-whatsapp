...
cr = conn.cursor()
cr.execute(\"\"\"
    SELECT count(*), whatsapp_status 
    FROM mailing_trace 
    WHERE trace_type = 'whatsapp' 
    GROUP BY whatsapp_status;
\"\"\")
print("Status dos Traces no banco:")
...
cr.execute(\"\"\"
    ...
    WHERE mailing_type = 'whatsapp' AND state IN ('in_queue', 'sending');
\"\"\")
print("Campanhas em fila ou enviando:")
rows = cr.fetchall()
print(rows if rows else "NENHUMA campanha em fila ou disparando no momento!")
conn.close()
"""
