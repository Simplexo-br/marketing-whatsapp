...
with registry.cursor() as cr:
    ...
    tpl = Template.search([('name', '=', 'simplexo_erp_vendas_pdv_v1')], limit=1)
    if tpl:
        print(f"Template sincronizado no Odoo com sucesso! ID: {tpl.id} | Nome: {tpl.name} | Status: {tpl.status}")
    else:
        print("Template não encontrado após sync!")
    cr.commit()
"""
