...
with open('/opt/odoo/addons/simplexo_aios_whatsapp/models/whatsapp_message.py.bak_webhook', 'r') as f:
    lines = f.readlines()
    print("=== whatsapp_message.py.bak_webhook (linhas 490-570) ===")
    for i in range(489, min(570, len(lines))):
        print(f"{i+1}: {lines[i]}", end='')

    print("\\n=== whatsapp_message.py.bak_webhook (linhas 570-660) ===")
    for i in range(569, min(660, len(lines))):
        print(f"{i+1}: {lines[i]}", end='')
"""
