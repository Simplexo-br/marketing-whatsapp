...
for p in ['/opt/odoo/addons/simplexo_aios_whatsapp', '/opt/odoo/addons/simplexo_ai_assistant_aios']:
    cmd = f"git -C {p} log -n 5 --oneline 2>/dev/null"
    res = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, text=True)
    print(f"=== GIT LOG {p} ===")
    print(res.stdout or "Não é repositório git separado")
"""
