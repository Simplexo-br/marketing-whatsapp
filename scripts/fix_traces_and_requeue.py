...
def extract_valid_phone(raw):
    ...
    if digits.startswith('55') and len(digits) in [12, 13]:
        ddd = int(digits[2:4])
        if ddd in VALID_BRAZILIAN_DDDS:
            return f"+{digits}"
...
# 1. Buscar todos os traces da campanha 134
cr.execute(\"\"\"
    SELECT id, whatsapp_recipient_number, whatsapp_status
    FROM mailing_trace
    WHERE mass_mailing_id = 134;
\"\"\")
all_traces = cr.fetchall()
print(f"Total traces: {len(all_traces)}")

updated_count = 0
requeued_count = 0

for tid, num, status in all_traces:
    digits_len = len(re.sub(r'[^0-9]', '', str(num or '')))
    new_phone = None
    if digits_len > 13:
        new_phone = extract_valid_phone(num)
        
    if new_phone and new_phone != num:
        cr.execute(\"\"\"
            UPDATE mailing_trace
            SET whatsapp_recipient_number = %s
            WHERE id = %s;
        \"\"\", (new_phone, tid))
        updated_count += 1
...
conn.commit()
print(f"Traces normalizados com telefone válido: {updated_count}")
print(f"Traces recuperados e re-enfileirados de falhas: {requeued_count}")
...
\"\"\")
print("Status final dos traces:", cr.fetchall())
conn.close()
"""
